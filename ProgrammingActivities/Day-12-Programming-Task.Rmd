...
* Now you're going to transform `One`, `Two` and `Three` to create correlations between the two columns. 
    - Create an matrix `Red` that is `One` times the Cholesky decomposition of `covar_red`.  The `Red` matrix will contain correlated random variables with a covariance of approximately `covar_red`.
