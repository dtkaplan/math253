...
* 1-dimensional homogeneous collections of numbers, booleans/logicals or of character strings.  These collections are called *vectors*    
...
```
    ...
    - Operations, e.g. `sum()`, `mean()`, `max()`, `table()`, `cumsum()`, arithmetic and mathematical functions (e.g. `sqrt()`), ...
    - Index vectors with `[ ]`. The index can be a single or multiple numbers in the range 1 to `length(v)`. It can also be a logical vector of length `length(v)`.
```{r}
funny <- ceiling(3*runif(10))
my_vector[funny]
length(my_vector)
dim(my_vector)
```
